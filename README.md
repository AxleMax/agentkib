...
- `agenthub-mcp` 是独立 stdio 服务；安装后无需保持桌面 App 运行。
- macOS 菜单栏图标常驻，后台每 15 分钟刷新资产索引。第一次关闭窗口会询问隐藏到菜单栏或退出；隐藏后 Dock 不再显示 AgentHub，只有菜单栏“退出 AgentHub”或将关闭行为改为“退出应用”才会结束进程。
...
```bash
cargo run -p agenthub-cli -- scan /path/to/project
cargo run -p agenthub-cli -- context /path/to/project codex
cargo run -p agenthub-cli -- validate /path/to/project
```
