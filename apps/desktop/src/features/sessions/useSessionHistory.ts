...
export function useSessionHistory(
  ...
  const read = useCallback(async () => {
    ...
    try {
      const page = await api.sessionEvents(sessionId);
      if (!isCurrent()) return;
      ...
      if (isCurrent()) setState({ ...emptyHistory(key), error: localizeMessage(reason) });
    } finally {
  ...
  const loadEarlier = useCallback(async () => {
    ...
    const isCurrent = () => activeKey.current === key && sequence.current === request;
    try {
      const page = await api.sessionEvents(sessionId, cursor);
      if (!isCurrent()) return;
      availableCursor.current = page.next_cursor;
      setState((current) => ({
        ...current,
        events: uniqueEvents([...page.events, ...current.events]),
        warnings: [...new Set([...page.warnings, ...current.warnings])],
        nextCursor: page.next_cursor,
  ...
  return { ...visible, loadEarlier, retry: read };
}
