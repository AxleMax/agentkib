...
type AppSearch = {
  ...
  workspaceAssetSection?: WorkspaceAssetSection;
...
export function AppShell() {
  ...
  const globalPage = route.kind === "global" ? route.page : "home";
  ...
  const assetSection = search.assetSection ?? "instructions";
  const workspaceAssetSection = search.workspaceAssetSection ?? "instructions";
  const workspaceView = search.workspaceView ?? "list";
  const insightsSection = search.insightsSection ?? "overview";
  ...
  const {
    ...
    navigationRequest, setNavigationRequest, menuCommand, setMenuCommand, refreshJobs, setRefreshJobs,
  } = appStore;
  const {
    project, setProject, selectedWorkspace, setSelectedWorkspace, scan, setScan, manifest, setManifest,
    changeSet, setChangeSet, changeSetOrigin, setChangeSetOrigin, handoffLaunchRequest, setHandoffLaunchRequest,
    baselineManifest, setBaselineManifest, workspaceDrafts, setWorkspaceDrafts, message, setMessage,
    busy, setBusy, applyingChanges, setApplyingChanges,
  } = workspaceStore;
  ...
  const navigateWorkspacePage = (nextPage: Page) => {
    if (route.kind !== "workspace") return;
    navigateWorkspacePageFor(route.workspaceId, nextPage);
  };
  ...
  const setPage = (nextPage: Page) => navigateWorkspacePage(nextPage);
  ...
  const setAssetSection = (nextSection: AssetSection) => updateSearch({ assetSection: nextSection });
  const setWorkspaceAssetSection = (nextSection: WorkspaceAssetSection) => updateSearch({ workspaceAssetSection: nextSection });
  const setWorkspaceView = (nextView: WorkspaceView) => updateSearch({ workspaceView: nextView });
  const setInsightsSection = (nextSection: InsightsSection) => updateSearch({ insightsSection: nextSection });
