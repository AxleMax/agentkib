...
export function AppShell() {
  ...
  const setSettingsSection = (nextSection: SettingsSection) =>
    updateSearch({ settingsSection: nextSection });
  const setQuotaProvider = (nextProvider: string | undefined) =>
    updateSearch({ quotaProvider: nextProvider });
  const setQuotaWindow = (nextWindow: QuotaWindowSelector | undefined) =>
    updateSearch({ quotaWindow: nextWindow });
        ...
        <span className="min-w-0 flex-1">
          ...
          {insights.total_tokens || insights.my_commits ? (
            <span className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-sm font-semibold">
              <strong>{formatCompact(insights.total_tokens)} Token</strong>
              <strong>
