...
function testRuntime(trayAvailable: boolean): RuntimeInfo {
  return {
    ...
    mcp_hub: {
      ...
      port: 47831,
      lan_enabled: false,
      accessible_addresses: ["127.0.0.1"],
      runtime_count: 0,
      error_count: 0,
    },
...
vi.mock("@tauri-apps/api/event", () => ({
  listen: vi
    .fn()
    .mockImplementation((event: string, handler: (event: { payload: unknown }) => void) => {
...
vi.mock("../core/api", () => ({
  api: {
    ...
    obsidianIntegration: vi.fn().mockResolvedValue({
    ...
    setLocale: vi.fn().mockImplementation((locale: string) =>
      Promise.resolve({
        close_behavior: "minimize-to-tray",
        locale_preference: locale,
        effective_locale: locale === "system" ? "en-US" : locale,
        theme_preference: "system",
        effective_theme: "dark",
        app_icon_preference: "white",
      }),
    ),
    setThemePreference: vi.fn().mockImplementation((preference: string) =>
      Promise.resolve({
        close_behavior: "minimize-to-tray",
        locale_preference: "system",
        effective_locale: "en-US",
        theme_preference: preference,
        effective_theme: preference === "light" ? "light" : "dark",
        app_icon_preference: "white",
      }),
    ),
    setAppIconPreference: vi
      .fn()
      .mockImplementation((preference: string) =>
        Promise.resolve({ ...testRuntime(true), app_icon_preference: preference }),
      ),
    ...
    setSessionIndexEnabled: vi
      .fn()
      .mockImplementation((enabled: boolean) =>
        Promise.resolve({ ...testRuntime(true), session_index_enabled: enabled }),
      ),
    workspaceOpeners: vi
      .fn()
      .mockResolvedValue([
    ...
    requestRefresh: vi
      .fn()
      .mockResolvedValue({ kind: "discovery", disposition: "queued", request_id: "test-refresh" }),
    refreshStatus: vi.fn().mockResolvedValue([]),
    storageOverview: vi.fn().mockResolvedValue({
    ...
    discoverWorkspaces: vi
      .fn()
      .mockResolvedValue({ kind: "discovery", disposition: "queued", request_id: "test-refresh" }),
    insightsSummary: vi.fn().mockResolvedValue({
      total_tokens: 120000,
      input_tokens: 80000,
      output_tokens: 40000,
      cache_tokens: 10000,
      ...
      coverage_to: "2026-08-13",
    }),
    insightsHeatmap: vi.fn().mockResolvedValue([
      {
        date: "2026-08-13",
        tokens: 120000,
        my_commits: 8,
    ...
    agentUsageBreakdown: vi.fn().mockResolvedValue([
      {
        agent: "codex",
        total_tokens: 120000,
        input_tokens: 80000,
    ...
    modelUsageBreakdown: vi
      .fn()
      .mockResolvedValue([{ model: "gpt-5", total_tokens: 120000, session_count: 12 }]),
    workspaceUsageBreakdown: vi
      .fn()
      .mockResolvedValue([{ name: "未关联工作区", total_tokens: 120000, session_count: 12 }]),
    ...
    quotaCollectorStatus: vi.fn().mockResolvedValue({
    ...
    quotaPopoverPreferences: vi
      .fn()
      .mockResolvedValue({ hidden_providers: [], hidden_windows: [] }),
    setQuotaPopoverPreferences: vi
      .fn()
    ...
    refreshQuota: vi
      .fn()
      .mockResolvedValue({ kind: "quota", disposition: "queued", request_id: "quota-refresh" }),
    insightsView: vi.fn().mockResolvedValue({
      ...
      repositories: [],
      achievements: [],
      status: { providers: [], running: false },
    }),
...
describe("AgentKib desktop", () => {
  ...
  it("applies a ChangeSet once and clears it after success", async () => {
    ...
    fireEvent.change(await screen.findByLabelText("Shared Project Instructions"), {
  ...
  it("applies a handoff once and retries only the Agent launch after partial success", async () => {
    ...
    const handoffChangeSet = {
      ...
      changes: [
        {
          target: "/tmp/project/.agentkib/handoffs/handoff.md",
    ...
    vi.mocked(api.agentInstallations).mockResolvedValue([
      { agent: "claude-code", installed: true, configured: true, warnings: [] },
    ]);
    vi.mocked(api.refreshWorkspaceSessions).mockResolvedValue([
      {
        id: "session",
    ...
    vi.mocked(api.planSessionHandoff).mockResolvedValue({
      change_set: handoffChangeSet,
      launch_request: launchRequest,
    });
    let finishContinuation!: (result: {
      status: "applied-launch-failed";
      error: { key: string; detail: string };
    }) => void;
    vi.mocked(api.continueSessionHandoff).mockImplementation(
      () =>
        new Promise((resolve) => {
    ...
    const continueButton = await screen.findByRole("button", {
      name: "Apply and continue in Claude Code",
    });
    ...
    expect(
      await screen.findByText("The handoff was saved, but the Agent could not be opened"),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Apply and continue in Claude Code" }),
  ...
  it("counts only Doctor errors and warnings on Home", async () => {
    ...
    expect(
      within(attention.closest("section")!).getByText("3 configuration warnings"),
    ).toBeInTheDocument();
  });
  ...
  it("discards a Doctor repair plan after switching workspaces", async () => {
      ...
      issues: [
        {
          ...
          repairable: true,
          evidence: [{ path: "/tmp/first/AGENTS.md", detail: "missing" }],
        },
      ],
    });
    let finishPlan!: (value: Awaited<ReturnType<typeof api.plan>>) => void;
    vi.mocked(api.plan).mockImplementation(
      () =>
        new Promise((resolve) => {
          finishPlan = resolve;
        }),
    ...
    await waitFor(() =>
      expect(api.plan).toHaveBeenCalledWith("/tmp/first", expect.anything(), false),
    );
    ...
    await act(async () =>
      finishPlan({
        id: "stale-repair",
        project_root: "/tmp/first",
        created_at: "2026-08-18T00:00:00Z",
        requires_home_approval: false,
        changes: [
          {
            target: "/tmp/first/AGENTS.md",
            scope: "project",
            before: "",
            after: "fixed",
            risk: "low",
    ...
    vi.mocked(api.workspaces).mockResolvedValue([
      {
        ...
        sources: [{ agent: "codex", evidence: "session-cwd", session_count: 2 }],
      },
      {
        ...
        sources: [{ agent: "claude-code", evidence: "session-cwd", session_count: 1 }],
      },
    ]);
    renderApp();
  ...
  it("defers discovery cache reads while the window is hidden", async () => {
    ...
    act(() =>
      tauriListeners.get("agentkib:refresh-state")?.({
        payload: { kind: "discovery", state: "succeeded" },
      }),
    );
  ...
  it("handles native menu navigation and Settings sections", async () => {
    ...
    act(() =>
      tauriListeners.get("agentkib:navigate")?.({
        payload: { page: "settings", settings_section: "diagnostics" },
      }),
    );
  ...
  it("reuses the existing workspace picker for native menu commands", async () => {
    ...
    act(() =>
      tauriListeners.get("agentkib:app-command")?.({ payload: { command: "add-workspace" } }),
    );
  ...
  it("maps native refresh commands to the active page", async () => {
    ...
    act(() =>
      tauriListeners.get("agentkib:app-command")?.({ payload: { command: "refresh-current" } }),
    );
  ...
  it("confirms before refreshing every data source from the native menu", async () => {
    ...
    act(() =>
      tauriListeners.get("agentkib:app-command")?.({ payload: { command: "refresh-all" } }),
    );
    fireEvent.click(await screen.findByRole("button", { name: "Confirm" }));
  ...
  it("asks before quitting with an unsaved workspace draft", async () => {
    ...
    fireEvent.click(await screen.findByRole("button", { name: /Project \/tmp\/project/ }));
    fireEvent.click(await screen.findByRole("tab", { name: "Assets" }));
    fireEvent.change(screen.getByRole("textbox", { name: "Shared Project Instructions" }), {
  ...
  it("renders only the panels for the selected achievement section", async () => {
    ...
    fireEvent.click(
      within(screen.getByRole("tablist", { name: "Achievements" })).getByRole("tab", {
        name: "Token",
      }),
    );
  ...
  it("renders a unified achievement wall and opens track and secret details", async () => {
    ...
    vi.mocked(api.insightsView).mockResolvedValue({
      ...
      achievements: [
        ...
        {
          code: "special-comeback",
          category: "special",
          threshold: 1,
          progress: 1,
          unlocked_at: "2026-08-02T00:00:00Z",
        },
      ],
    });
    renderApp();
    ...
    await waitFor(() =>
      expect(within(trackDialog).getAllByRole("button")).toContain(document.activeElement),
    );
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "View achievement details: Mystery Achievement" }),
    );
  ...
  it("disables background hiding when the system tray is unavailable", async () => {
    ...
    expect(
      await screen.findByRole("option", { name: /Hide in (Menu Bar|System Tray)/ }),
  ...
  it.runIf(["darwin", "windows"].includes(import.meta.env.TAURI_ENV_PLATFORM))(
    "opens system file access settings from Data & Privacy",
    async () => {
      renderApp();
      await waitForHome();
      fireEvent.click(screen.getByRole("button", { name: "Settings" }));
  ...
  it("collapses the sidebar completely and restores the preference", async () => {
    ...
    expect(collapse.closest("[data-tauri-drag-region]")).toHaveAttribute(
    ...
    expect(
      screen.getByRole("button", { name: "Expand sidebar" }).closest("[data-tauri-drag-region]"),
    ).toBeInTheDocument();
    first.unmount();
    renderApp();
    await waitFor(() =>
