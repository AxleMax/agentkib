...
export function QuotaDiagnostics({ status }: { status?: QuotaCollectorStatus }) {
  ...
  return <div className="divide-y divide-border/60">
    ...
    {status.error_key && <div className="flex flex-col gap-2 bg-destructive/5 px-5 py-4 text-sm text-destructive"><strong>{tr(status.error_key)}</strong>{status.error_detail && <Collapsible><CollapsibleTrigger className="text-xs font-medium underline-offset-4 hover:underline">{tr("common.details")}</CollapsibleTrigger><CollapsibleContent><pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap rounded-md bg-background/70 p-3 text-xs text-muted-foreground">{status.error_detail}</pre></CollapsibleContent></Collapsible>}</div>}
  </div>;
}

function DiagnosticRow({ label, value }: { label: string; value: string }) { return <div className="flex min-h-14 items-center justify-between gap-5 px-5 py-3.5 text-sm"><span className="text-muted-foreground">{label}</span><strong className="text-right font-medium">{value}</strong></div>; }
