...
function AchievementWallCard({ item, onOpen }: { item: AchievementWallItem; onOpen: () => void }) {
  if (item.kind === "track") {
    ...
    return <Button variant="bare" size="content" className={`achievement-wall-card track${item.unlocked ? " unlocked" : " locked"}`} onClick={onOpen} aria-label={tr("achievementWall.openTrack", { category: tr(`milestones.category.${item.track.category}`) })}><span className="achievement-wall-icon"><Icon size={20} /></span><span className="achievement-wall-kind">{tr(`milestones.category.${item.track.category}`)}</span><strong>{title}</strong><small>{formatMilestoneValue(item.track.category, item.cover.threshold)}</small><span className="achievement-wall-footer"><span>{tr("milestones.completed", { completed: item.track.completed, total: item.track.milestones.length })}</span><ChevronRight size={15} /></span></Button>;
  }
...
function AchievementDetailDialog({ item, onClose }: { item: AchievementWallItem; onClose: () => void }) {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className="achievement-dialog" showCloseButton={false}><header><div><span>{item.kind === "track" ? tr("achievementWall.track") : tr("special.title")}</span><DialogTitle id="achievement-dialog-title">{title}</DialogTitle></div><Button className="ghost icon-only" onClick={onClose} aria-label={tr("common.close")}><X size={17} /></Button></header>{item.kind === "track" ? <AchievementTrackDetail track={item.track} /> : <SpecialAchievementDetail item={item} />}</DialogContent></Dialog>;
}
