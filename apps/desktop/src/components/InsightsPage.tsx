...
export function InsightsPage({ section, workspaces, onSummary }: { section: InsightsSection; workspaces: WorkspaceSummary[]; onSummary: (summary: InsightsSummary) => void }) {
  ...
  return <div className="stack insights-page">
    ...
    <div className="insights-filters">
      {showTokenFilters && <SelectControl aria-label={tr("workspace.allAgents")} value={agent} onChange={(event) => setAgent(event.target.value as typeof agent)}><option value="all">{tr("workspace.allAgents")}</option>{Object.entries(agentLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</SelectControl>}
    ...
    {summary && section === "overview" && <>
      ...
      <Card className="panel heatmap-panel"><CardHeader className="panel-head"><h2>{tr("insights.heatmap")}</h2></CardHeader><CardContent className="p-0"><Tabs value={metric} onValueChange={(value) => setMetric(value as HeatmapMetric)}><TabsList className="w-fit max-w-full justify-start gap-1 overflow-x-auto rounded-none border-b border-border bg-transparent px-[17px]" variant="line" aria-label={tr("insights.heatmap")}>{(Object.keys(metricLabels) as HeatmapMetric[]).map((value) => <TabsTrigger className="flex-none rounded-none px-3" key={value} value={value}>{metricLabels[value]}</TabsTrigger>)}</TabsList></Tabs><div className="heatmap-scroll"><HeatmapMonths points={points} padding={padding} /><div className="heatmap-grid">{Array.from({ length: padding }, (_, index) => <span className="heatmap-cell empty-cell" key={`padding-${index}`} />)}{points.map((point) => { const value = point[metric]; const level = value ? Math.max(1, Math.ceil(value / max * 4)) : 0; return <span key={point.date} className={`heatmap-cell level-${level}`} title={`${point.date} · ${metricLabels[metric]} ${formatCompact(value)}`} />; })}</div></div><div className="heatmap-legend"><span>{tr("insights.less")}</span>{[0,1,2,3,4].map((level) => <i key={level} className={`heatmap-cell level-${level}`} />)}<span>{tr("insights.more")}</span></div></CardContent></Card>
    </>}
