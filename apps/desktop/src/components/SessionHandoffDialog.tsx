    ...
    <section className="panel handoff-dialog" role="dialog" aria-modal="true" aria-label={tr("handoff.title")}>
      ...
      {draft ? <div className="handoff-preview">
        ...
        <div className="handoff-fields">
          <label className="wide">{tr("handoff.target")}<select value={targetAgent} disabled={busy} onChange={(event) => setTargetAgent(event.target.value as AgentKind)}>{availableTargets.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
