...
function Harness() {
  ...
  return (
    <div>
      <Button
        onClick={() =>
          void dialogs
            ...
            .then((accepted) => (document.body.dataset.first = String(accepted)))
        }
      >
        First confirm
      </Button>
      <Button
        onClick={() =>
          void dialogs
            ...
            .then((accepted) => (document.body.dataset.second = String(accepted)))
        }
      >
        Second confirm
      </Button>
      ...
      <Button
        onClick={() =>
          void dialogs.notify("Saved locally").then(() => (document.body.dataset.notified = "true"))
        }
      >
        Notify
      </Button>
      <Button
        onClick={() =>
          void dialogs
            .requestSecrets(["API_KEY", "TOKEN"])
            .then(
              (values) =>
                (document.body.dataset.secrets = values ? JSON.stringify(values) : "cancelled"),
            )
        }
      >
        Secrets
...
describe("AppDialogProvider", () => {
  ...
  it("clears secret values after cancel before reopening", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Confirm" }));
    expect(JSON.parse(document.body.dataset.secrets ?? "{}")).toEqual({
      API_KEY: "new-key",
      TOKEN: "",
    });
  });
