...
export function WindowToolbar({
  ...
  return (
    <div
      className="pointer-events-auto fixed inset-x-auto left-0 top-0 z-[45] h-[var(--page-header-height)] w-[var(--sidebar-width)] bg-transparent"
      data-tauri-drag-region
    >
      <Button
        size="icon"
        className={cn(
          "absolute top-[9px] z-[1] grid place-items-center rounded-[7px] bg-transparent p-0 text-muted-foreground hover:bg-muted hover:text-foreground",
          toolbarPosition,
        )}
        type="button"
        onClick={onToggle}
