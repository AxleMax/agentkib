...
export function ObsidianSettingsCard() {
  ...
  return (
    <Card className="overflow-hidden rounded-2xl border-border/70 shadow-sm">
      ...
      {integration ? <InstallationStatus integration={integration} /> : <p className="px-5 py-3 text-sm text-muted-foreground">{tr("common.loading")}</p>}
      {integration?.installation.app_path && <code className="mx-5 mb-3 block break-all text-xs text-muted-foreground">{integration.installation.app_path}</code>}
      {error && <div className="mx-5 my-3 flex items-center gap-2 rounded-lg border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">{error}</div>}
      <div className="px-5 pb-5 pt-2.5">
        ...
        {integration?.vaults.map((vault) => (
          <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2.5 border-t border-border/60 py-3" key={vault.path}>
...
export function WorkspaceObsidianCard({ workspaceId }: { workspaceId: string }) {
  ...
  return (
    <Card className="overflow-hidden rounded-2xl border-border/70 pb-5 shadow-sm">
      ...
      {error && <div className="mx-5 mt-3 flex items-center gap-2 rounded-lg border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">{error}</div>}
      {!integration && <p className="px-5 py-3 text-sm text-muted-foreground">{tr("common.loading")}</p>}
      {integration && !integration.installation.installed && <p className="px-5 py-3 text-sm text-muted-foreground">{tr("obsidian.notInstalledWorkspace")}</p>}
