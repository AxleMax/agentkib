...
export function AgentsPage({ installations, assets, workspaces, remoteGateways, insightsStatus, onOpen }: { installations: AgentInstallation[]; assets: CatalogAsset[]; workspaces: WorkspaceSummary[]; remoteGateways: RemoteGatewaySummary[]; insightsStatus?: InsightsStatus; onOpen: (workspace: WorkspaceSummary) => Promise<void> }) {
  ...
  return <div className="agent-master-detail">
    <Card className="panel agent-master-list">{agentKinds.map((agent) => { const item = installations.find((value) => value.agent === agent); const remoteCount = remoteGateways.filter((gateway) => gateway.kind === agent).reduce((total, gateway) => total + gateway.workspaces.length, 0); const count = workspaces.filter((workspace) => workspace.sources.some((source) => source.agent === agent)).length + remoteCount; return <Button variant="bare" size="content" key={agent} className={selected === agent ? "active" : ""} onClick={() => { setSelected(agent); setSection("overview"); }}><AgentIcon agent={agent} /><span><strong>{agentLabels[agent]}{agent === "deepseek-harness" && <Badge variant="outline">Beta</Badge>}</strong><small>{count} {tr("common.workspaces")}</small></span><Badge variant={item?.installed ? "secondary" : "outline"}>{tr(item?.installed ? "common.installed" : item?.configured ? "agents.localDataFound" : "common.notInstalled")}</Badge><ChevronRight size={15} /></Button>; })}</Card>
    <Card className="panel agent-detail"><div className="agent-detail-head"><AgentIcon agent={selected} /><h2>{agentLabels[selected]}</h2>{selected === "deepseek-harness" && <Badge variant="outline">Beta</Badge>}{installation?.version && <span className="agent-version">{installation.version}</span>}</div><Tabs value={section} onValueChange={(value) => setSection(value as AgentDetailSection)}><TabsList className="section-tabs product-tabs agent-detail-tabs" variant="line" aria-label={agentLabels[selected]}>{(["overview", "assets", "workspaces", "usage"] as AgentDetailSection[]).map((value) => <TabsTrigger value={value} key={value}>{tr(`agents.section.${value}`)}</TabsTrigger>)}</TabsList></Tabs>
      {section === "overview" && <>{selected === "deepseek-harness" && <div className="agent-beta-note">{tr("agents.deepseekReadOnly")}</div>}<div className="agent-facts three"><div><span>{tr("agents.linkedWorkspaces")}</span><strong>{linkedWorkspaces.length + remoteWorkspaceCount}</strong></div><div><span>{tr("agents.homeAssets")}</span><strong>{homeAssets.length}</strong></div><div><span>{tr("agents.provider")}</span><strong>{provider?.available ? tr("quota.available") : tr("insights.noData")}</strong></div></div>{installation?.home && <div className="agent-home-path"><code>{installation.home}</code></div>}{installation?.warnings.map((warning) => <div className="warning" key={warning}><CircleAlert size={14} />{installationWarningLabel(warning)}</div>)}<div className="agent-overview-grid"><section className="detail-section"><h3>{tr("agents.recentWorkspaces")}</h3><div className="agent-workspace-list preview-list">{recentLinkedWorkspaces.map((workspace) => <Button variant="bare" size="content" key={workspace.id} onClick={() => void onOpen(workspace)}><FolderGit2 size={14} /><span>{workspace.name}<small title={workspace.path}>{workspace.path}</small></span><small>{workspace.last_active_at ? formatRelativeTime(workspace.last_active_at) : tr("common.never")}</small></Button>)}{!recentLinkedWorkspaces.length && <p className="neutral-empty">{tr("agents.noRecentWorkspaces")}</p>}</div></section><section className="detail-section"><h3>{tr("agents.homeAssetTypes")}</h3><dl className="summary-list compact-summary">{homeAssetKinds.map(([kind, count]) => <div key={kind}><dt>{tr(`status.asset.${kind}`)}</dt><dd>{count}</dd></div>)}{!homeAssetKinds.length && <div><dt>{tr("agents.noHomeAssets")}</dt><dd>0</dd></div>}</dl></section></div>{selectedRemoteGateways.length > 0 && <RemoteAgentGatewayDetails gateways={selectedRemoteGateways} />}</>}
      {section === "assets" && <div className="detail-section"><div className="toolbar compact-toolbar"><div className="search"><Search size={15} /><Input aria-label={tr("catalog.searchPlaceholder")} value={assetQuery} onChange={(event) => setAssetQuery(event.target.value)} placeholder={tr("catalog.searchPlaceholder")} /></div>{assetKinds.length > 1 && <SelectControl aria-label={tr("catalog.allTypes")} className="setting-select" value={assetKind} onChange={(event) => setAssetKind(event.target.value)}><option value="all">{tr("catalog.allTypes")}</option>{assetKinds.map((value) => <option key={value} value={value}>{tr(`status.asset.${value}`)}</option>)}</SelectControl>}</div><div className="home-asset-list">{visibleHomeAssets.map((asset) => <div key={asset.id}><FileCode2 size={14} /><span><strong>{asset.name}</strong><small>{shortPath(asset.path)}</small></span><em>{tr(`status.asset.${asset.kind}`)}</em></div>)}{!visibleHomeAssets.length && <Empty icon={FileCode2} title={tr("agents.noHomeAssets")} />}</div></div>}
