...
function WorkspacesPage({ view, storageJob, workspaces, assetCounts, discoveryRefreshing, onAddWorkspace, onViewChange, onOpen, onRefreshDiscovery, onRefreshWorkspace, onExclude }: { view: WorkspaceView; storageJob?: RefreshJobStatus; workspaces: WorkspaceSummary[]; assetCounts: Map<string, number>; discoveryRefreshing: boolean; onAddWorkspace: () => void; onViewChange: (view: WorkspaceView) => void; onOpen: (workspace: WorkspaceSummary) => Promise<void>; onRefreshDiscovery: () => Promise<void>; onRefreshWorkspace: (id: string) => Promise<void>; onExclude: (id: string) => Promise<void> }) {
  ...
  const pageIntro = <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
    <div>
      <p className="text-[11px] font-semibold uppercase tracking-[.16em] text-muted-foreground">{tr("nav.workspaces")}</p>
      <h1 className="mt-1 text-2xl font-semibold tracking-[-.035em] text-foreground">{tr("nav.workspaces")}</h1>
      <p className="mt-1 text-sm text-muted-foreground">{tr("workspace.resultCount", { count: filtered.length })}</p>
    </div>
    {viewControls}
  </div>;
  const filterBar = <Card className="rounded-2xl border-border/70 bg-card shadow-sm">
    ...
    <div className="grid gap-5">
      ...
      <Card className="overflow-hidden rounded-2xl border-border/70 bg-card shadow-sm">
        <CardContent className="p-0">
          <Table className="min-w-[900px] border-separate border-spacing-0 [&_th]:h-[44px] [&_th]:bg-muted/25 [&_th]:text-[11px] [&_th]:font-semibold [&_th]:uppercase [&_th]:tracking-[.08em] [&_th]:text-muted-foreground [&_th]:whitespace-nowrap [&_th:first-child]:pl-5 [&_td]:h-[76px] [&_td]:text-sm [&_tr]:transition-colors">
