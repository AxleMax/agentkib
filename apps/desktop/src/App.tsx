...
function WorkspacesPage({ view, storageJob, workspaces, assetCounts, discoveryRefreshing, onOpen, onRefreshDiscovery, onRefreshWorkspace, onExclude }: { view: WorkspaceView; storageJob?: RefreshJobStatus; workspaces: WorkspaceSummary[]; assetCounts: Map<string, number>; discoveryRefreshing: boolean; onOpen: (workspace: WorkspaceSummary) => Promise<void>; onRefreshDiscovery: () => Promise<void>; onRefreshWorkspace: (id: string) => Promise<void>; onExclude: (id: string) => Promise<void> }) {
  ...
  return <div className="workspace-index"><div className="panel workspace-table"><div className="toolbar sticky-toolbar workspace-list-toolbar"><div className="search"><Search size={16} /><input aria-label={tr("workspace.searchPlaceholder")} value={query} onChange={(event) => setQuery(event.target.value)} placeholder={tr("workspace.searchPlaceholder")} /></div><div className="toolbar-filters"><select aria-label={tr("workspace.allAgents")} className="setting-select" value={agent} onChange={(event) => setAgent(event.target.value as typeof agent)}><option value="all">{tr("workspace.allAgents")}</option>{Object.entries(agentLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select><select aria-label={tr("workspace.allStatuses")} className="setting-select" value={status} onChange={(event) => setStatus(event.target.value as typeof status)}><option value="all">{tr("workspace.allStatuses")}</option><option value="healthy">{workspaceStatusLabel("healthy")}</option><option value="attention">{workspaceStatusLabel("attention")}</option></select></div><span className="result-count">{tr("workspace.resultCount", { count: filtered.length })}</span><button className="ghost icon-only workspace-refresh" title={tr("workspace.refreshDiscovery")} aria-label={tr("workspace.refreshDiscovery")} aria-busy={discoveryRefreshing} onClick={() => void onRefreshDiscovery()} disabled={discoveryRefreshing}><RefreshCw size={15} className={discoveryRefreshing ? "spin" : ""} /></button></div><div className="workspace-column-head" aria-hidden="true"><span>{tr("workspace.projectColumn")}</span><span>{tr("workspace.agentColumn")}</span><span>{tr("workspace.assetsColumn")}</span><span>{tr("workspace.activityColumn")}</span><span /></div><div className="repository-groups">{groups.map((group) => { const grouped = group.length > 1; return <section className={grouped ? "workspace-repository-group" : "single-workspace-group"} key={group[0].repository_group_id ?? group[0].id}>{grouped && <header><FolderGit2 size={15} /><strong>{group[0].name}</strong><span>{tr("workspace.worktrees", { count: group.length })}</span></header>}{group.map((workspace) => <div className="workspace-card" key={workspace.id}><WorkspaceRow mode="columns" workspace={workspace} assetCount={assetCounts.get(workspace.id)} onOpen={onOpen} /><details className="row-menu"><summary title={tr("common.moreActions")} aria-label={`${workspace.name} · ${tr("common.moreActions")}`}><MoreHorizontal size={15} /></summary><div><button className="menu-neutral" onClick={() => void onRefreshWorkspace(workspace.id)}><RefreshCw size={13} />{tr("common.scan")}</button><button onClick={() => void onExclude(workspace.id)}><Trash2 size={13} />{tr("workspace.ignore")}</button></div></details></div>)}</section>; })}{!groups.length && <Empty compact icon={FolderGit2} title={tr("workspace.noMatch")} text={tr("workspace.noMatchText")} />}</div></div></div>;
}
...
function ConversationPrivacySettings({ runtime, workspaces, onChanged }: { runtime?: RuntimeInfo; workspaces: WorkspaceSummary[]; onChanged: (runtime: RuntimeInfo) => void }) {
  ...
  const clear = async () => {
    if (!window.confirm(tr("conversations.clearConfirm"))) return;
    setBusy(true); setError("");
  ...
  return <SettingGroup title={tr("conversations.settingsTitle")}>
    <div className="setting-row"><div><strong>{tr("conversations.indexSetting")}</strong></div><label className="switch"><input type="checkbox" checked={runtime?.session_index_enabled !== false} disabled={busy} onChange={(event) => void toggle(event.target.checked)} /><span /></label></div>
