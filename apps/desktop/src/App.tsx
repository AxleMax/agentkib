  ...
  if (selectedWorkspace && project && scan) return (
    <div className={shellClass}>
      ...
      <main className={cn(mainClass, page === "git" && "workspace-git-main")}>
        <header className={cn(pageHeaderClass, "workspace-header !h-[58px] !min-h-[58px]")} data-tauri-drag-region><div className={pageTitleClass}><h1 className="!m-0 !text-[23px] !leading-none" data-tauri-drag-region>{selectedWorkspace.name}</h1></div></header>
        {message && <div className="alert"><CircleAlert size={17} />{message}</div>}
        <div className="workspace-tab-row"><Tabs value={page} onValueChange={(id) => { if (id !== "git") setGitSubview(undefined); setPage(id as Page); }}><TabsList className="workspace-tabs product-tabs" variant="line" aria-label={selectedWorkspace.name}>{workspaceTabs.map(([id, label, Icon]) => <TabsTrigger key={id} value={id}><Icon size={15} />{tr(label)}{id === "changes" && changeSet?.changes.length ? <em>{changeSet.changes.length}</em> : null}</TabsTrigger>)}</TabsList></Tabs><WorkspaceActions workspace={selectedWorkspace} onError={setMessage} onScan={() => load(project, manifest)} busy={busy} onReview={() => plan(false)} reviewDisabled={busy || !hasUnsavedDraft} /></div>
...
function DeferredPage({ children }: { children: ReactNode }) {
  return <Suspense fallback={<Card className="panel"><p>{tr("common.loading")}</p></Card>}>{children}</Suspense>;
}
