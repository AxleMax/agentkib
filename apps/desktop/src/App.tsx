...
export function App() {
  ...
  if (selectedWorkspace && project && scan) return (
    <div className={shellClass}>
      ...
      <main className={cn(mainClass, page === "git" && "workspace-git-main")}>
        <header className={cn(pageHeaderClass, "workspace-header !h-[58px] !min-h-[58px]")} data-tauri-drag-region />
        {message && <div className="alert"><CircleAlert size={17} />{message}</div>}
        <div className="workspace-tab-row"><Tabs value={page} onValueChange={(id) => { if (id !== "git") setGitSubview(undefined); setPage(id as Page); }}><TabsList className="w-full justify-start gap-1 overflow-x-auto rounded-none border-b border-border bg-transparent px-7 py-0" variant="line" aria-label={selectedWorkspace.name}>{workspaceTabs.map(([id, label, Icon]) => <TabsTrigger className="min-h-11 flex-none rounded-none px-3" key={id} value={id}><Icon size={15} />{tr(label)}{id === "changes" && changeSet?.changes.length ? <em>{changeSet.changes.length}</em> : null}</TabsTrigger>)}</TabsList></Tabs><WorkspaceActions workspace={selectedWorkspace} onError={setMessage} onScan={() => load(project, manifest)} busy={busy} onReview={() => plan(false)} reviewDisabled={busy || !hasUnsavedDraft} /></div>
        <section className={`${contentClass} !pt-[34px] workspace-content${page === "git" ? " workspace-git-content" : ""}`}>
  ...
  return <div className={`${shellClass} global-shell`}><WindowToolbar collapsed={sidebarCollapsed} onToggle={() => setSidebarCollapsed((value) => !value)} /><AppSidebar active={globalPage} entries={navigation} collapsed={sidebarCollapsed} onNavigate={navigateGlobal} onSettings={openSettings} />{!sidebarCollapsed && <Button className="sidebar-backdrop" type="button" aria-label={tr("common.closeSidebar")} onClick={() => setSidebarCollapsed(true)} />}<main className={mainClass}><header className={pageHeaderClass} data-tauri-drag-region />{message && <div className="alert"><CircleAlert size={17} />{message}</div>}{globalPage === "workspaces" && discoveryFailure?.error && <div className="alert"><CircleAlert size={17} />{discoveryFailure.error}</div>}<section className={`${contentClass} global-content`}>
    ...
    {globalPage === "insights" && <div className="insights-host" data-view={insightsSection}><div className="insights-tab-bar"><Tabs value={insightsSection} onValueChange={(section) => setInsightsSection(section as InsightsSection)}><TabsList className="w-full justify-start gap-1 overflow-x-auto rounded-none border-b border-border bg-transparent pr-[58px]" variant="line" aria-label={tr("nav.insights")}>{(["overview", "tokens", "commits", "milestones", "sources"] as InsightsSection[]).map((section) => <TabsTrigger className="flex-none rounded-none px-3" key={section} value={section}>{tr(`insights.section.${section}`)}</TabsTrigger>)}</TabsList></Tabs><Button className="ghost icon-only insights-tab-refresh" aria-label={tr("insights.refresh")} title={tr("insights.refresh")} onClick={() => void refreshCurrentView()} disabled={insightsRefreshing}><RefreshCw size={15} className={insightsRefreshing ? "spin" : ""} /></Button></div><DeferredPage><InsightsPageLazy section={insightsSection} workspaces={workspaces} onSummary={setInsightsSummary} /></DeferredPage></div>}
  </section></main></div>;
...
function WorkspaceActions({ workspace, onError, onScan, busy, onReview, reviewDisabled }: { workspace: WorkspaceSummary; onError: (message: string) => void; onScan: () => void | Promise<void>; busy: boolean; onReview: () => void | Promise<void>; reviewDisabled: boolean }) {
  return <div className="workspace-action-bar"><div className="header-actions"><span className="workspace-status attention" hidden={workspace.status !== "attention"}>{workspaceStatusLabel("attention")}</span><WorkspaceOpenWith workspace={workspace} onError={onError} /><DropdownMenu><DropdownMenuTrigger className="row-menu-trigger header-menu" title={tr("common.moreActions")} aria-label={tr("common.moreActions")}><MoreHorizontal size={16} /></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => void navigator.clipboard?.writeText(workspace.path)}><Copy size={13} />{tr("workspace.copyPath")}</DropdownMenuItem></DropdownMenuContent></DropdownMenu><Button className="ghost icon-only" title={tr("common.scan")} aria-label={tr("common.scan")} onClick={() => void onScan()} disabled={busy}><RefreshCw size={15} className={busy ? "spin" : ""} /></Button><Button className="primary" onClick={() => void onReview()} disabled={reviewDisabled}><GitCompareArrows size={15} />{tr("workspace.reviewChanges")}</Button></div></div>;
}
...
function GlobalHome({ workspaces, doctorSummaries, installations, memories, discovery, activity, insights, uniqueAssetCount, assetCounts, onShowInsights, onShowWorkspaces, onShowAgents, onOpen, onOpenDoctor, onOpenAssets, onAddRoot }: { workspaces: WorkspaceSummary[]; doctorSummaries: Record<string, ContextDoctorSummary>; installations: AgentInstallation[]; memories: MemoryRecord[]; discovery?: DiscoveryReport; activity: ActivityRecord[]; insights?: InsightsSummary; uniqueAssetCount: number; assetCounts: Map<string, number>; onShowInsights: () => void; onShowWorkspaces: () => void; onShowAgents: () => void; onOpen: (workspace: WorkspaceSummary) => Promise<void>; onOpenDoctor: (workspace: WorkspaceSummary) => Promise<void>; onOpenAssets: (section: AssetSection) => void; onAddRoot: () => Promise<void> }) {
  ...
  const insightCard = insights && <Card className="panel home-achievement" role="button" tabIndex={0} onClick={onShowInsights} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onShowInsights(); }}><div className="achievement-orb"><Award size={21} /></div><div><span>{tr("home.journey")}</span>{insights.total_tokens || insights.my_commits ? <div className="home-achievement-values"><strong>{formatCompact(insights.total_tokens)} Token</strong><strong>{insights.my_commits} {tr("insights.myCommits")}</strong></div> : <h2>{tr("home.insightsEmpty")}</h2>}<p>{tr("home.streak", { active: insights.active_days, current: insights.current_streak, longest: insights.longest_streak })}</p></div><ChevronRight size={16} /></Card>;
  return <div className="stack home-dashboard">
    {issueCount > 0 ? <Card className="attention-panel has-issues"><div className="attention-heading"><span><CircleAlert size={18} /><strong>{tr("home.needsAttention")}</strong></span><Badge variant="secondary">{issueCount}</Badge></div><div className="attention-items">{attention.slice(0, 4).map((workspace) => { const doctorCount = (doctorSummaries[workspace.id]?.error_count ?? 0) + (doctorSummaries[workspace.id]?.warning_count ?? 0); return <Button variant="bare" size="content" key={workspace.id} onClick={() => void onOpenDoctor(workspace)}><ShieldCheck size={15} /><span><strong>{workspace.name}</strong><small>{tr("home.workspaceWarnings", { count: doctorCount || workspace.warning_count })}</small></span><ChevronRight size={14} /></Button>; })}{pending > 0 && <Button variant="bare" size="content" onClick={() => onOpenAssets("memory")}><Brain size={15} /><span><strong>{tr("home.pendingMemory")}</strong><small>{tr("home.pendingMemoryDetail", { count: pending })}</small></span><ChevronRight size={14} /></Button>}</div></Card> : <div className="attention-clear compact"><Check size={18} /><strong>{tr("home.allClear")}</strong></div>}
    <Card className="summary-strip three"><Button variant="bare" size="content" onClick={onShowWorkspaces}><span>{tr("home.workspaceMetric")}</span><strong>{workspaces.length}</strong></Button><Button variant="bare" size="content" onClick={() => onOpenAssets("instructions")}><span>{tr("home.assetMetric")}</span><strong>{uniqueAssetCount}</strong></Button><Button variant="bare" size="content" onClick={onShowAgents}><span>{tr("home.installedAgents")}</span><strong>{installations.filter((item) => item.installed).length}</strong></Button></Card>
    {!workspaces.length ? <>{insightCard}<Card className="panel empty-global"><FolderGit2 size={30} /><h2>{tr("home.emptyTitle")}</h2><p>{tr("home.emptyText")}</p><Button className="primary" onClick={() => void onAddRoot()}>{tr("home.addScanRoot")}</Button></Card></> : <div className={`home-main-grid${!insightCard && !importantActivity.length ? " single-column" : ""}`}><Card className="panel"><CardHeader className="panel-head"><h2>{tr("home.recentWorkspaces")}</h2><Badge variant="outline">{discovery ? tr("home.updated", { time: relativeTime(discovery.finished_at) }) : tr("home.discovering")}</Badge></CardHeader><CardContent className="p-0"><div className="workspace-list">{workspaces.slice(0, 5).map((workspace) => <WorkspaceRow key={workspace.id} workspace={workspace} assetCount={assetCounts.get(workspace.id)} onOpen={onOpen} />)}</div></CardContent></Card>{(insightCard || importantActivity.length > 0) && <div className="home-side-stack">{insightCard}{importantActivity.length > 0 && <Card className="panel"><CardHeader className="panel-head"><h2>{tr("home.recentActivity")}</h2></CardHeader><CardContent className="p-0"><div className="activity-list compact">{importantActivity.map((item) => <ActivityRow key={item.id} record={item} />)}</div></CardContent></Card>}</div>}</div>}
  </div>;
}
...
function WorkspacesPage({ view, storageJob, workspaces, assetCounts, discoveryRefreshing, onAddWorkspace, onViewChange, onOpen, onRefreshDiscovery, onRefreshWorkspace, onExclude }: { view: WorkspaceView; storageJob?: RefreshJobStatus; workspaces: WorkspaceSummary[]; assetCounts: Map<string, number>; discoveryRefreshing: boolean; onAddWorkspace: () => void; onViewChange: (view: WorkspaceView) => void; onOpen: (workspace: WorkspaceSummary) => Promise<void>; onRefreshDiscovery: () => Promise<void>; onRefreshWorkspace: (id: string) => Promise<void>; onExclude: (id: string) => Promise<void> }) {
  ...
  const viewControls = <div className="flex items-center justify-end gap-2 pb-3">
    <ToggleGroup className="theme-segments workspace-view-switch" value={[view]} onValueChange={(values) => { const value = values[0]; if (value === "list" || value === "storage") onViewChange(value); }} aria-label={tr("workspace.viewLabel")}>
      <ToggleGroupItem value="list" className={view === "list" ? "active" : ""}>{tr("workspace.view.list")}</ToggleGroupItem>
      <ToggleGroupItem value="storage" className={view === "storage" ? "active" : ""}>{tr("workspace.view.storage")}</ToggleGroupItem>
    </ToggleGroup>
    <Button className={cn("primary", view !== "list" && "invisible pointer-events-none")} aria-hidden={view !== "list"} disabled={view !== "list"} onClick={onAddWorkspace}><FolderGit2 size={15} />{tr("workspace.addManually")}</Button>
  </div>;
  if (view === "storage") return <div className="workspace-index">{viewControls}<WorkspaceStoragePage workspaces={workspaces} job={storageJob} /></div>;
  return (
    <div className="workspace-index">
      {viewControls}
      <Card className="panel workspace-table workspace-table-card">
        <CardHeader className="toolbar sticky-toolbar workspace-list-toolbar !flex-row !items-center !justify-between !p-[10px_14px]">
          <div className="search">
            <Search size={16} />
            <Input aria-label={tr("workspace.searchPlaceholder")} value={query} onChange={(event) => setQuery(event.target.value)} placeholder={tr("workspace.searchPlaceholder")} />
          </div>
          <div className="toolbar-filters">
            <SelectControl aria-label={tr("workspace.allAgents")} className="setting-select" value={agent} onChange={(event) => setAgent(event.target.value as typeof agent)}>
            ...
            <SelectControl aria-label={tr("workspace.allStatuses")} className="setting-select" value={status} onChange={(event) => setStatus(event.target.value as typeof status)}>
          ...
          <span className="result-count">{tr("workspace.resultCount", { count: filtered.length })}</span>
          <Button className="ghost icon-only workspace-refresh" title={tr("workspace.refreshDiscovery")} aria-label={tr("workspace.refreshDiscovery")} aria-busy={discoveryRefreshing} onClick={() => void onRefreshDiscovery()} disabled={discoveryRefreshing}><RefreshCw size={15} className={discoveryRefreshing ? "spin" : ""} /></Button>
        </CardHeader>
        <CardContent className="p-0">
          <Table className="workspace-data-table">
            <TableHeader className="workspace-table-head"><TableRow><TableHead>{tr("workspace.projectColumn")}</TableHead><TableHead>{tr("workspace.agentColumn")}</TableHead><TableHead>{tr("workspace.assetsColumn")}</TableHead><TableHead>{tr("workspace.activityColumn")}</TableHead><TableHead className="w-12" /></TableRow></TableHeader>
            <TableBody>
              {groups.flatMap((group) => {
                ...
                return grouped ? [<TableRow className="workspace-group-row" key={`${group[0].id}:group`}><TableCell colSpan={5}><FolderGit2 size={15} /><strong>{group[0].name}</strong><span>{tr("workspace.worktrees", { count: group.length })}</span></TableCell></TableRow>, ...rows] : rows;
              })}
            </TableBody>
          </Table>
...
function WorkspaceTableRow({ workspace, assetCount, onOpen, onRefresh, onExclude }: { workspace: WorkspaceSummary; assetCount?: number; onOpen: (workspace: WorkspaceSummary) => Promise<void>; onRefresh: (id: string) => Promise<void>; onExclude: (id: string) => Promise<void> }) {
  ...
  return <TableRow className="workspace-data-row">
    <TableCell><Button variant="bare" size="content" className="workspace-table-project" aria-label={`${workspace.name} · ${workspace.path}`} onClick={() => void onOpen(workspace)}><span className="workspace-project-icon" aria-hidden="true"><FolderGit2 size={16} /></span><span className="workspace-project-copy"><strong>{workspace.name}</strong><small title={workspace.path}>{workspace.path}</small></span></Button></TableCell>
    <TableCell><div className="workspace-table-agents" aria-label={agents}>{sourceAgents.length ? sourceAgents.map((value) => <Badge variant="secondary" key={value}>{agentLabels[value]}</Badge>) : <Badge variant="secondary">{agents}</Badge>}</div></TableCell>
    <TableCell><strong className={`workspace-asset-count${count ? "" : " is-zero"}${count ? "" : " text-muted-foreground"}`}>{count}</strong></TableCell>
    <TableCell className="workspace-activity-cell">{workspace.last_active_at ? relativeTime(workspace.last_active_at) : tr("common.never")}</TableCell>
    <TableCell><div className="workspace-table-actions">{workspace.status === "attention" && <Badge className="workspace-status attention" variant="destructive">{workspaceStatusLabel("attention")}</Badge>}<DropdownMenu><DropdownMenuTrigger className="row-menu-trigger" title={tr("common.moreActions")} aria-label={`${workspace.name} · ${tr("common.moreActions")}`}><MoreHorizontal size={15} /></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => void onRefresh(workspace.id)}><RefreshCw size={13} />{tr("common.scan")}</DropdownMenuItem><DropdownMenuItem variant="destructive" onClick={() => void onExclude(workspace.id)}><Trash2 size={13} />{tr("workspace.ignore")}</DropdownMenuItem></DropdownMenuContent></DropdownMenu><ChevronRight size={15} /></div></TableCell>
  </TableRow>;
}
...
function GlobalMemoryInbox({ records, workspaces, onReload }: { records: MemoryRecord[]; workspaces: WorkspaceSummary[]; onReload: () => Promise<void> }) {
  ...
  return <Card className="panel inbox"><CardHeader className="panel-head"><div><h2>{tr("memory.globalTitle")}</h2><p>{tr("memory.globalPending", { count: records.filter((item) => item.status === "pending").length })}</p></div></CardHeader><CardContent className="p-0"><div className="memory-list">{records.map((record) => <div key={record.id} className="global-memory-item"><span className="workspace-memory-label">{workspaces.find((item) => item.manifest_workspace_id === record.project_id)?.name ?? record.project_id.slice(0, 8)}</span><MemoryCard record={record} onReview={review} /></div>)}{!records.length && <Empty icon={Brain} title={tr("memory.empty")} text={tr("memory.globalEmptyText")} />}</div></CardContent></Card>;
}

function ActivityPage({ records }: { records: ActivityRecord[] }) { return <Card className="panel"><CardHeader className="panel-head"><div><h2>{tr("activity.title")}</h2><p>{tr("activity.description")}</p></div></CardHeader><CardContent className="p-0"><div className="activity-list">{records.map((record) => <ActivityRow key={record.id} record={record} />)}{!records.length && <Empty icon={History} title={tr("home.noActivity")} text={tr("activity.emptyText")} />}</div></CardContent></Card>; }
...
function GlobalSettings({ section, runtime, workspaces, discovery, insightsStatus, quotaStatus, remoteGateways, scanRoots, excluded, activity, onAddRoot, onRemoveRoot, onRestore, onCloseBehaviorChanged, onLocaleChanged, onRemoteGatewaysChanged }: { section: SettingsSection; runtime?: RuntimeInfo; workspaces: WorkspaceSummary[]; discovery?: DiscoveryReport; insightsStatus?: InsightsStatus; quotaStatus?: QuotaCollectorStatus; remoteGateways: RemoteGatewaySummary[]; scanRoots: ScanRoot[]; excluded: ExcludedWorkspace[]; activity: ActivityRecord[]; onAddRoot: () => Promise<void>; onRemoveRoot: (id: string) => Promise<void>; onRestore: (path: string) => Promise<void>; onCloseBehaviorChanged: (behavior?: CloseBehavior) => Promise<void>; onLocaleChanged: (runtime: RuntimeInfo) => void; onRemoteGatewaysChanged: () => Promise<void> }) {
  if (section === "general") return <div className="grid gap-6"><Card className="overflow-hidden"><CardContent className="p-0"><ThemeSetting runtime={runtime} onChanged={onLocaleChanged} /><AppIconSetting runtime={runtime} onChanged={onLocaleChanged} /><LanguageSetting runtime={runtime} onChanged={onLocaleChanged} /><div className="setting-row"><div><strong>{tr("settings.closeBehavior")}</strong></div><CloseBehaviorSelect value={runtime?.close_behavior} trayAvailable={runtime?.tray_available !== false} onChange={onCloseBehaviorChanged} /></div>{runtime?.tray_available === false && <div className="setting-detail warning" role="status"><CircleAlert size={14} />{tr("settings.trayUnavailable")}</div>}</CardContent></Card></div>;
  if (section === "discovery") return <div className="grid gap-6"><SettingGroup title={tr("settings.discovery")}><div className="setting-row"><div><strong>{tr("settings.discoveryStatus")}</strong></div><span className={discovery?.errors.length ? "status rejected" : "ready"}>{discovery ? tr("settings.workspaceCount", { count: discovery.discovered_count }) : tr("home.discovering")}</span></div>{discovery?.errors.map((error) => <div className="setting-detail error" key={error}>{error}</div>)}</SettingGroup><Card className="overflow-hidden"><CardHeader className="flex flex-row items-center justify-between border-b border-border-subtle px-4 py-4"><CardTitle className="text-base">{tr("settings.scanRoots")}</CardTitle><Button className="primary" onClick={() => void onAddRoot()}>{tr("settings.addFolder")}</Button></CardHeader><CardContent className="p-0"><div className="settings-list">{scanRoots.map((root) => <div key={root.id}><FolderGit2 size={16} /><span><strong>{root.path}</strong><small>{tr("settings.maxDepth", { depth: root.max_depth })}</small></span><Button className="icon-danger" onClick={() => void onRemoveRoot(root.id)}><Trash2 size={15} /></Button></div>)}{!scanRoots.length && <p>{tr("settings.noScanRoots")}</p>}</div></CardContent></Card><Card className="overflow-hidden"><CardHeader className="px-4 py-4"><CardTitle className="text-base">{tr("settings.excluded")}</CardTitle></CardHeader><CardContent className="p-0"><div className="settings-list">{excluded.map((item) => <div key={item.path}><X size={16} /><span><strong>{item.path}</strong><small>{formatDateTime(item.created_at)}</small></span><Button className="ghost" onClick={() => void onRestore(item.path)}>{tr("common.restore")}</Button></div>)}{!excluded.length && <p>{tr("settings.noExcluded")}</p>}</div></CardContent></Card></div>;
  if (section === "integrations") return <div className="grid gap-6"><SettingGroup title="AgentKib MCP Hub"><div className="setting-row"><div><strong>{tr("mcp.network")}</strong><code>{runtime?.mcp_hub ? runtime.mcp_hub.accessible_addresses.join(" · ") : "—"}</code></div><span className={runtime?.mcp_hub?.running ? "ready" : "status neutral"}>{tr(runtime?.mcp_hub?.running ? "mcp.running" : "mcp.stopped")}</span></div></SettingGroup><RemoteGatewaysSettings gateways={remoteGateways} onChanged={onRemoteGatewaysChanged} /><ObsidianSettingsCard /></div>;
...
function Changes({ changeSet, origin, launchRequest, onPlanHome, onApplied, onLaunchCompleted, onRejected, onApplyingChange }: { changeSet?: ChangeSet; origin: ChangeSetOrigin; launchRequest?: SessionHandoffLaunchRequest; onPlanHome: () => void; onApplied: (keepLaunchRequest?: boolean) => void | Promise<void>; onLaunchCompleted: () => void; onRejected: () => void; onApplyingChange: (applying: boolean) => void }) {
  ...
  if (!changeSet && launchRequest && appliedLaunchFailure) return <Card className="panel handoff-launch-result"><CircleAlert size={24} /><div><h2>{tr("handoff.savedLaunchFailed")}</h2><p>{error || appliedLaunchFailure}</p><code>.agentkib/handoffs/{launchRequest.filename}</code></div><Button className="primary" disabled={busy} onClick={() => void runLocked(async () => { await api.launchSessionHandoff(launchRequest); if (!active.current) return; setAppliedLaunchFailure(""); onLaunchCompleted(); })}><ExternalLink size={15} />{tr(busy ? "handoff.opening" : "handoff.retryOpen", { agent: targetAgentName })}</Button></Card>;
