...
function SettingGroup({ title, children }: { title: string; children: ReactNode }) { return <Card className="panel settings-section"><div className="panel-head"><h2>{title}</h2></div><div className="setting-rows">{children}</div></Card>; }
...
function Overview({ workspace, scan, manifest }: { workspace: WorkspaceSummary; scan: WorkspaceScan; manifest: Manifest }) {
  ...
  return <div className="stack">
    ...
    <div className="workspace-overview-grid"><Card className="panel"><CardHeader className="panel-head"><h2>{tr("overview.publicSource")}</h2>{scan.manifest_exists && <Badge variant="outline">Schema v{manifest.schema_version}</Badge>}</CardHeader><CardContent className="p-0"><dl className="summary-list"><div><dt>{tr("assets.sharedInstructions")}</dt><dd>{manifest.instructions.shared.trim() ? 1 : 0}</dd></div><div><dt>{tr("overview.sharedSkills")}</dt><dd>{manifest.skills.length}</dd></div><div><dt>MCP</dt><dd>{manifest.connections.length}</dd></div><div><dt>{tr("overview.scopedRules")}</dt><dd>{manifest.instructions.scoped.length}</dd></div></dl></CardContent></Card><Card className="panel"><CardHeader className="panel-head"><h2>{tr("overview.projectAgentConfigs")}</h2></CardHeader><CardContent className="p-0"><div className="agent-readiness-list">{configuredAgents.map((agent) => <div key={agent.agent}><AgentIcon agent={agent.agent} /><strong>{agentLabels[agent.agent]}</strong><span>{tr("overview.nativeAssets", { count: agent.asset_count })}</span><Badge variant={agent.warnings.length ? "destructive" : "secondary"}>{tr(agent.warnings.length ? "status.workspace.attention" : "overview.detected")}</Badge></div>)}{!configuredAgents.length && <p className="neutral-empty">{tr("overview.noProjectAgentConfigs")}</p>}{unconfiguredAgents.length > 0 && <Collapsible className="unconfigured-agents"><CollapsibleTrigger>{tr("overview.otherAgents", { count: unconfiguredAgents.length })}</CollapsibleTrigger><CollapsibleContent>{unconfiguredAgents.map((agent) => <div key={agent.agent}><AgentIcon agent={agent.agent} /><strong>{agentLabels[agent.agent]}</strong><span>{tr("overview.noProjectConfig")}</span></div>)}</CollapsibleContent></Collapsible>}</div></CardContent></Card></div>
