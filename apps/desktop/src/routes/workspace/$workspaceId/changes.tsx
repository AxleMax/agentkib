...
import { Brain, Check, CircleAlert, ExternalLink, FileCode2, GitCompareArrows, ShieldCheck } from "lucide-react";
...
function WorkspaceChangesRoute() {
  ...
  const { project, changeSet, changeSetOrigin, handoffLaunchRequest, setChangeSet, setHandoffLaunchRequest, setApplyingChanges, setScan, setManifest, setBaselineManifest, setProject, setMessage } = useWorkspaceStore();
  ...
  const reload = async () => {
    const [scan, manifest, runtime] = await Promise.all([api.scan(project), api.manifest(project), api.runtime()]);
    setProject(project);
    setScan(scan);
  ...
  return <Changes changeSet={changeSet} origin={changeSetOrigin} launchRequest={handoffLaunchRequest} onPlanHome={() => void planHome()} onApplied={async (keepLaunchRequest) => { setChangeSet(undefined); if (!keepLaunchRequest) setHandoffLaunchRequest(undefined); try { await reload(); } catch (error) { setMessage(String(error)); } }} onLaunchCompleted={() => setHandoffLaunchRequest(undefined)} onRejected={() => { setChangeSet(undefined); setHandoffLaunchRequest(undefined); void navigate({ to: "/workspace/$workspaceId", params: { workspaceId } }); }} onApplyingChange={setApplyingChanges} />;
}

export const Route = createFileRoute("/workspace/$workspaceId/changes")({ component: WorkspaceChangesRoute });
