...
function WorkspaceAssetsRoute() {
  ...
  const setSection = (nextSection: WorkspaceAssetSection) => void navigate({ to: "/workspace/$workspaceId/assets", params: { workspaceId }, search: (current) => ({ ...current, workspaceAssetSection: nextSection }) as never });
  if (!scan || !manifest) return <LoadingState label="Loading…" />;
  return <Assets section={section} onSection={setSection} scan={scan} manifest={manifest} onChange={setManifest} />;
}

export const Route = createFileRoute("/workspace/$workspaceId/assets")({ component: WorkspaceAssetsRoute });
