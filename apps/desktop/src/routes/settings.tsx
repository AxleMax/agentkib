...
function SettingsRoute() {
  ...
  const addRoot = () => run(async () => {
    const selected = await open({ directory: true, multiple: false, title: tr("dialog.addScanRoot") });
  ...
  const removeRoot = (id: string) => run(async () => {
    await api.removeScanRoot(id);
    setScanRoots(await api.scanRoots());
    await api.requestRefresh("discovery", true);
  });

  const restoreExcluded = (path: string) => run(async () => {
    await api.restoreExcludedWorkspace(path);
    setExcluded(await api.excludedWorkspaces());
    await api.requestRefresh("discovery", true);
  });

  const changeCloseBehavior = (behavior?: CloseBehavior) => run(async () => {
    await api.setCloseBehavior(behavior);
    setRuntime(await api.runtime());
  });

  const refreshRemoteGateways = () => run(async () => {
    setRemoteGateways(await api.remoteGateways());
  });

  const changeRuntime = (nextRuntime: RuntimeInfo) => setRuntime(nextRuntime);

  return <div className={cn("group app-shell !grid !h-full !w-full !min-h-0 !overflow-hidden !grid-cols-[var(--sidebar-width)_minmax(0,1fr)] !grid-rows-[minmax(0,1fr)] !transition-[grid-template-columns] !duration-150", sidebarCollapsed && "sidebar-collapsed !grid-cols-[0_minmax(0,1fr)]")}>
    ...
    <SettingsSidebar active={section} collapsed={sidebarCollapsed} platform={platform} onSelect={setSection} onBack={() => void navigate({ to: "/" })} />
    ...
    <main className={cn("!col-start-2 !row-start-1 !min-h-0 !min-w-0 !h-full !overflow-x-hidden !overflow-y-auto !overscroll-contain !text-sm", `settings-section-${section}`)}>
      <header className={cn("page-header !sticky !top-0 !z-10 !flex !min-h-[58px] !h-[58px] !items-center !justify-between !border-b !border-[var(--page-header-border)] !bg-[var(--page-header-background)] !pr-7", sidebarCollapsed ? "!pl-[132px]" : "!pl-7")} data-tauri-drag-region />
      ...
      <section className={cn("mx-auto grid w-full max-w-[1180px] gap-5 px-7 pb-10 pt-[22px] max-[900px]:px-[18px]", section === "general" && "pt-4")}>
        <GlobalSettings section={section} runtime={runtime} workspaces={workspaces} discovery={discovery} insightsStatus={insightsStatus} quotaStatus={quotaStatus} remoteGateways={remoteGateways} scanRoots={scanRoots} excluded={excluded} activity={activity} onAddRoot={addRoot} onRemoveRoot={removeRoot} onRestore={restoreExcluded} onCloseBehaviorChanged={changeCloseBehavior} onLocaleChanged={changeRuntime} onRemoteGatewaysChanged={refreshRemoteGateways} />
      </section>
    </main>
  </div>;
}
