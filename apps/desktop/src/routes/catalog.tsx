...
import { Boxes, Brain, Check, CircleAlert, ExternalLink, FileCode2, Library, Pencil, PlugZap, RefreshCw, Search, Sparkles, Trash2, X } from "lucide-react";
import type { AgentKind, ChangeSet, CatalogAsset, Manifest, McpInstallation, McpRegistryEntry, McpRuntimeStatus, McpServerConfig, MemoryRecord, MemoryType, RuntimeInfo, WorkspaceSummary } from "../core/types";
...
function CatalogRoute() {
  ...
  const setSection = (nextSection: AssetSection) => {
    void navigate({ to: "/catalog", search: (current) => ({ ...current, assetSection: nextSection }) as never });
  };
  const reload = async () => {
    const [nextAssets, nextMemories, nextWorkspaces, nextRuntime] = await Promise.all([api.catalogAssets(), api.globalMemories(), api.workspaces(), api.runtime()]);
    setCatalog(nextAssets); setGlobalMemories(nextMemories); setWorkspaces(nextWorkspaces); setRuntime(nextRuntime);
  };
  ...
  const planMigration = async (project: string, planned: ChangeSet) => {
    ...
    setChangeSet(planned); setChangeSetOrigin("standard"); setHandoffLaunchRequest(undefined);
    await navigate({ to: "/workspace/$workspaceId/changes", params: { workspaceId: workspace.id } });
  };

  return <CatalogPage section={section} onSection={setSection} assets={assets} workspaces={workspaces} memories={memories} runtime={runtime} onReload={reload} onRuntimeChanged={setRuntime} onOpen={(id) => void openWorkspaceById(id)} onMigrationPlanned={planMigration} />;
}

function CatalogPage({ section, onSection, assets, workspaces, memories, runtime, onReload, onRuntimeChanged, onOpen, onMigrationPlanned }: { section: AssetSection; onSection: (section: AssetSection) => void; assets: CatalogAsset[]; workspaces: WorkspaceSummary[]; memories: MemoryRecord[]; runtime?: RuntimeInfo; onReload: () => Promise<void>; onRuntimeChanged: (runtime: RuntimeInfo) => void; onOpen: (id: string) => void; onMigrationPlanned: (project: string, changeSet: ChangeSet) => Promise<void> }) {
  ...
  return <div className="relative grid gap-5 pb-8">
    ...
    {section === "instructions" && <AssetPage assets={instructionAssets} workspaces={workspaces} onOpen={onOpen} />}
    {section === "skills" && <AssetPage assets={skillAssets} workspaces={workspaces} onOpen={onOpen} />}
    {section === "other" && <AssetPage assets={otherAssets} workspaces={workspaces} onOpen={onOpen} />}
...
function CatalogEmpty({ title, text }: { title: string; text: string }) { return <div className="grid min-h-[260px] place-content-center justify-items-center gap-1.5 p-[30px] text-center text-muted-foreground"><Brain size={28} className="mb-1.5" /><h3 className="m-0 text-[13px] font-semibold text-foreground">{title}</h3><p className="m-0 max-w-[380px] leading-relaxed">{text}</p></div>; }
