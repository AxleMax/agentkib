...
const assetsDirectory = join(desktopDirectory, "src-tauri/assets");
const iconsDirectory = join(desktopDirectory, "src-tauri/icons");
...
try {
  ...
  await copyFile(
    join(darkOutput, "128x128@2x.png"),
    join(iconsDirectory, "app-icon-black.png"),
  );
  await generateMacosRuntimeIcon(
    join(assetsDirectory, "app-icon-source.png"),
    "white",
  );
  await generateMacosRuntimeIcon(
    join(assetsDirectory, "app-icon-black.png"),
    "black",
  );
} finally {
